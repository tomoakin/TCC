...
Case 1: DE analysis of two-group count data with replicates by using the exact test 
(Robinson and Smyth, 2008 \cite{robinson3}) in \Rpackage{edgeR} coupled with TbT 
normalization (termed the TbT-\Rpackage{edgeR} combination). 
...
Case 2: DE analysis for two-group count data with replicates by using the exact test 
coupled with iterative DEGES/\Rpackage{edgeR} normalization (i.e., the 
iDEGES/\Rpackage{edgeR}-\Rpackage{edgeR} combination). This is an alternative pipeline designed to reduce 
...
Case 3: DE analysis for two-group count data without replicates by using the negative 
binomial (NB) test in \Rpackage{DESeq} coupled with iDEGES/\Rpackage{DESeq} 
normalization (i.e., the iDEGES/\Rpackage{DESeq}-\Rpackage{DESeq} combination). 
...
The \Rfunction{new} function has to be used to perform the main functionalities 
...
In this vignette, we will call the \Rfunction{set.seed} function for obtaining 
...
The \Rfunction{calcNormFators} function together with the above parameter settings 
...
When obtaining normalization factors from two-group count data without replicates, 
...
\Rpackage{TCC} provides some prototypes of DEGES-based pipelines for such data. 
...
\Rpackage{TCC}. The \Rfunction{calcNormFators} function together with the above 
...
with the \Rfunction{test.method} argument in \Rpackage{TCC}, those DE methods in 
...
in \Rpackage{edgeR} together with iDEGES/\Rpackage{edgeR} normalization factors 
(i.e., the iDEGES/\Rpackage{edgeR}-\Rpackage{edgeR} combination) 
...
The \Rfunction{plot} function generates an M-A plot, where "M" indicates the log-ratio 
...
\Robject{"bayseq"} options for the \Robject{test.method} argument. Here, we show a procedure 
for DE analysis using the NB test implemented in \Rpackage{DESeq} together with 
iDEGES/\Rpackage{DESeq} normalization factors 
(i.e., the iDEGES/\Rpackage{DESeq}-\Rpackage{DESeq} combination) for the hypothetical 
...
The ranked gene list after performing the DEGES/\Rpackage{edgeR}-\Rpackage{edgeR} 
...
\bibitem{sun} Sun J, Nishiyama T, Shimizu K, and Kadota K. \Rpackage{TCC}: an R package for comparing tag count data with robust normalization strategies. submitted
