...
\usepackage[a4paper,left=2cm,top=2.5cm,bottom=2cm,right=2cm,ignoreheadfoot]{geometry}
\usepackage[usenames,dvipsnames]{color}
\usepackage[dvips]{graphicx}
\usepackage{Sweave}
%\SweaveOpts{keep.source=TRUE,png=TRUE}
...
(TbT; Kadota et al., 2012 \cite{kadota}) for two-group RNA-seq data. The strategy 
(called DEGES) is to remove data that are potential differentially 
...
Sun et al., submitted \cite{sun}) provides users with a robust and accurate framework 
to perform differential expression analysis of tag count data. \Rpackage{TCC} 
provides integrated analysis pipelines with improved data normalization steps, 
compared with other packages such as \Rpackage{edgeR}, \Rpackage{DESeq}, and 
\Rpackage{baySeq}, by appropriately combining their functionalities. 
The package incorporates multi-step normalization methods whose strategy is 
to remove data that are potential DEGs before performing the data normalization.

Kadota et al. (2012) \cite{kadota} recently reported that the normalization methods 
implemented in R packages (such as \Rpackage{edgeR} (Robinson et al., 2010 \cite{robinson}), 
\Rpackage{DESeq} (Anders and Huber, 2010 \cite{anders}), and \Rpackage{baySeq} (Hardcastle 
and Kelly, 2010 \cite{hardcastle})) for differential expression (DE) analysis between samples 
...
current methods implicitly assume a balanced DE, wherein the numbers of highly 
and lowly expressed DE entities in samples are (nearly) equal. As a result, 
...
et al., 2012 \cite{kadota}), in which the TMM normalization method (Robinson and Oshlack, 
2010 \cite{robinson2}) is used in steps 1 and 3 and an empirical Bayesian method implemented 
in the \Rpackage{baySeq} package (Hardcastle and Kelly, 2010 \cite{hardcastle}) is used in 
...
For example, the TbT normalization method (Kadota et al., 2012 \cite{kadota}), which is 
a particular functionality of the \Rpackage{TCC} package (Sun et al., submitted \cite{sun}), 
consists of the TMM normalization method (Robinson and Oshlack, 2010 \cite{robinson2}) implemented 
in the \Rpackage{edgeR} package (Robinson et al., 2010 \cite{robinson}) and the empirical Bayesian 
method implemented in the \Rpackage{baySeq} package (Hardcastle and Kelly, 2010 \cite{hardcastle}). 
Therefore, please cite the appropriate references when you publish your results.
...
between two groups from tag count data consisting of $1,000$ genes and a total 
of six samples (each group has three biological replicates). The hypothetical 
count data (termed "\Robject{hypoData}") is stored in this package (for details, 
see section \ref{section-2-1}). We then describe the DE analysis of count data without 
replicates (i.e., two samples), using the data of the first and the fourth 
column of \Robject{hypoData} (Case 3). We recommend the use of commands in 
Cases 2 and 3.

Case 1: DE analysis of tag count data with replicates by using the exact test 
(Robinson and Smyth, 2008 \cite{robinson3}) in \Rpackage{edgeR} coupled with TbT normalization. 
The \Rpackage{TCC} package was originally designed with the TbT normalization 
method, and the original study (Kadota et al., 2012 \cite{kadota}) recommended this analysis 
pipeline. Note that a smaller sampling size (i.e., \Robject{samplesize = 100}) 
is used here to reduce the computation time, but a larger sampling size of 
around $10,000$ (i.e., \Robject{samplesize = 10000}) is recommended (Hardcastle 
and Kelly, 2010 \cite{hardcastle}). Suggested citations are as follows: \Rpackage{TCC} (Sun et al., 
submitted \cite{sun}), TbT (Kadota et al., 2012 \cite{kadota}), 
TMM (Robinson and Oshlack, 2010 \cite{robinson2}), 
\Rpackage{baySeq} (Hardcastle and Kelly, 2010 \cite{hardcastle}), and \Rpackage{edgeR} 
(Robinson et al., 2010 \cite{robinson}). For details, see section \ref{section-3-1-1}.
...
Case 2: DE analysis for tag count data with replicates by using the exact test 
coupled with iterative DEGES/\Rpackage{edgeR} normalization (i.e., 
iDEGES/\Rpackage{edgeR}). This is an alternative pipeline designed to reduce 
the runtime (approx. $20$ sec.), yet its performance is comparable to 
the above pipeline. Accordingly, we recommend using this pipeline as a default 
when analyzing tag count data with replicates. A notable advantage of this 
pipeline is that the multi-step normalization strategy only needs the methods 
implemented in the \Rpackage{edgeR} package. The suggested citations are as 
follows: \Rpackage{TCC} (Sun et al., submitted \cite{sun}), TMM (Robinson and Oshlack, 
2010 \cite{robinson2}), the exact test (Robinson and Smyth, 2008 \cite{robinson3}), and \Rpackage{edgeR} 
(Robinson et al., 2010 \cite{robinson}). For details, see section \ref{section-3-1-3}.
...
Case 3: DE analysis for tag count data without replicates by using the negative 
binomial (NB) test in \Rpackage{DESeq} coupled with iDEGES/\Rpackage{DESeq} 
normalization. A procedure using the data of the first and fourth columns of 
...
citations are as follows: \Rpackage{TCC} (Sun et al., submitted \cite{sun}) and 
\Rpackage{DESeq} (Anders and Huber, 2010 \cite{anders}). For details, 
...
of G1, and the remaining five columns are from G2), the \Robject{group} vector 
...
The new function has to be used to perform the main functionalities 
of \Rpackage{TCC}. This function constructs a \Rpackage{TCC} class object, 
...
It can be seen that 
$\Sexpr{nrow(hypoData) - nrow(tcc$count)} (= \Sexpr{nrow(hypoData)} -\Sexpr{ nrow(tcc$count)})$ 
...
\subsection{Normalization of count data with replicates}
\label{section-3-1}
...
The calculated normalization factors and the computation time 
...
(about $\Sexpr{sprintf("%.0f", tcc$DEGES$execution.time[3])}$ seconds) 
is due to the empirical 
Bayesian method in step 2. This method estimates an empirical distribution of 
the parameters of the NB distribution by bootstrapping from the input data; 
the default sample size is set to a suggested value of $10,000$ 
(i.e., \Robject{samplesize = 10000}). While the sampling size can be 
made smaller to reduce the computation time (e.g., \Robject{samplesize = 100}), 
the resulting normalization factors will vary from trial to trial. 
In this vignette, we will call the set.seed function for obtaining 
reproducible results (i.e., the \Robject{tcc\$norm.factors}, 
values) when using any random function. That is,
...
Of course, the procedure can be performed by using functions in \Rpackage{edgeR} 
and \Rpackage{baySeq}, instead of using the \Rfunction{calcNormFators} 
function in \Rpackage{TCC}. The \Rfunction{calcNormFators} function 
with the default parameter settings can be regarded as a wrapper 
...
Now let us describe an alternative approach that is roughly $400$-$500$ times 
...
an arbitrary false discovery rate (FDR) with a floor value of $P_{\mbox{\tiny DEG}}$. 
...
The \Rfunction{calcNormFators} function with above parameter settings 
...
An iterative version of DEGES/TbT (iDEGES/TbT) can be described 
...
with the above arguments can be regarded as a wrapper function for 
...
\subsection{Normalization of count data without replicates}
\label{section-3-2}

It is important to keep in mind that most R packages (including \Rpackage{edgeR},
...
to be accurately estimated to avoid spurious DE calls (Glaus et al., 2012 \cite{glaus}). 
...
When obtaining normalization factors from count data without replicates, 
users can select a total of four combinations (two normalization methods 
$\times$ two DEG identification methods) coupled with an arbitrary number 
...
A suggested pipeline (DEGES/\Rpackage{DESeq}) for obtaining normalization 
...
\Rpackage{TCC} provides DEGES-based normalization methods for such data. 
Here, we analyze another hypothetical three-group count matrix, the 
\Robject{hypoData\_mg} object, provided in \Rpackage{TCC}. 
...
\Robject{samplesize = 10000}) is recommended (Hardcastle and Kelly, 2010 \cite{hardcastle}).
...
corresponds to the model coefficients. The two models can be defined as follows:
...
The design matrix (\Robject{design}) can be constructed by using the model.matrix function. 
For the model coefficients (\Robject{coef}), the user should specify all the coefficients 
except for the intercept term. The GLM-based DEGES/\Rpackage{edgeR} pipeline 
can be performed by adding the two arguments (\Robject{design} and \Robject{coef}) as follows:
...
\Rpackage{TCC}. The \Rfunction{calcNormFators} function with the above 
parameter settings can be regarded as a wrapper function for the 
following commands.
...
for the effective library sizes are adjusted to zero. Our normalization 
...
(ii) the first $180$ genes of the $200$ DEGs are higher in G1 (PG1 $= 180/200 = 90\%$), 
...
of the similarity of their summary statistics for {\it normalized} data labeled as 
non-DEGs in one group (e.g., G1) to those of the other group (e.g., G2). 
...
The \Rfunction{getNormalizedData} function can be applied to the \Rpackage{TCC} 
class object after the normalization factors have been calculated.
...
(ranging from $\Sexpr{sprintf("%.5f", min(apply(normalized.count[nonDEG,],2,median)))}$ 
...
$\Sexpr{sprintf("%.1f",max(hypoData.median))}$). For comparison, the summary statistics for 
...
are closer to the truth (i.e., $\Sexpr{sprintf("%.1f", hypoData.median[1])}$ for G1\_rep1 
...
with the test.method argument in \Rpackage{TCC}, those DE methods in 
\Rpackage{edgeR}, \Rpackage{DESeq}, and \Rpackage{baySeq} can be performed 
...
We give a pipeline for DE analysis using the exact test implemented 
in \Rpackage{edgeR} with iDEGES/\Rpackage{edgeR} normalization factors 
...
genes having an FDR threshold of $< 10\%$ as DEGs, he or she can do as follows.
...
The plot function generates an M-A plot, where "M" indicates the log-ratio 
...
in \Rpackage{baySeq} with iDEGES/\Rpackage{edgeR} normalization 
factors, he or she can do as follows.
...
used here to reduce the computation time, but a larger sampling size of around 
$10,000$ (i.e., \Robject{samplesize = 10000}) is recommended (Hardcastle 
and Kelly, 2010 \cite{hardcastle}). Note also that \Rpackage{baySeq} outputs posterior 
...
\Robject{"bayseq"} options for the \Robject{test.method} argument. Here, we show a pipeline 
for DE analysis using the NB test implemented in \Rpackage{DESeq} with 
iDEGES/\Rpackage{DESeq} normalization factors for the hypothetical 
two-group count data without replicates (i.e., 
the \Robject{hypoData[, c(1, 4)]} object). If the user wants to determine 
the genes having an FDR threshold of $< 10\%$ as DEGs, 
he or she can do as follows.
...
the DEGES/\Rpackage{edgeR} normalization can be performed as follows.
...
because the two arguments indicated here (\Robject{design} and \Robject{coef}) correspond 
...
iDEGES/\Rpackage{edgeR} normalization can be performed as follows.
...
real data (Dillies et al., 2012 \cite{dillies}). This means we have to evaluate the 
potential performance of our DEGES-based methods using mainly simulation 
data. The \Rfunction{simulateReadCounts} function generates simulation data under 
various conditions. This function can generate simulation data analyzed 
in the TbT paper (Kadota et al., 2012 \cite{kadota}), and that means it enables other 
...
As mentioned in section \ref{section-3-1-1}, we can call the \Rfunction{set.seed} 
function in order to obtain reproducible results 
(i.e., the \Robject{tcc\$count}) with the \Rfunction{simulateReadCounts} function. 
...
$1,000$ (i.e., \Robject{Ngene = 1000}), (ii) the first $20\%$ of genes are DEGs 
...
and (v) there are a total of six samples (three biological replicates for G1 and three 
biological replicates for G2) (\Robject{replicates = c(3, 3)}). 
The variance of the NB distribution can be modeled as  $V = \mu + \phi\mu^{2}$. 
...
Specifically, the entries for $0, 1$, and $2$ in the \Robject{tcc\$simulation\$trueDEG} 
...
DE method such as the exact test (Robinson and Smyth, 2008 \cite{robinson3}) and the 
...
The ranked gene list can be obtained as follows.
...
The following classic \Rpackage{edgeR} pipelines make it clear that 
the DEGES-based normalization method (i.e., 
...
(\ref{section-3-1-2}), 
the AUC value ($\Sexpr{sprintf("%.7f", auc.degesedger)}$) of DEGES/\Rpackage{edgeR} is quite similar 
to the AUC value ($\Sexpr{sprintf("%.7f", auc.degestbt)}$) of the original TbT method (i.e., DEGES/TbT):
...
the same as those in the previous section (i.e., \ref{section-5-2}), 
...
in both groups (\Robject{DEG.foldchange = c(4, 4)}), and (v) there are a total 
...
Now let us see how DEGES/\Rpackage{DESeq} with the original \Rpackage{DESeq} 
procedure performs. First, we calculate the AUC value for the ranked 
gene list obtained from the NB test in \Rpackage{DESeq} coupled with 
the DEGES/\Rpackage{DESeq} normalization.
...
It can be seen that DEGES/\Rpackage{DESeq} outperforms the original procedure 
...
$210$, $60$, and $30$ genes are up-regulated in G1, G2, and G3, respectively.
...
for non-DEGs based on the iDEGES/\Rpackage{edgeR}-normalized data is nearly zero. 
...
\bibitem{anders} Anders S and Huber W. Differential expression analysis for sequence count data. Genome Biol. 11(10): R106, 2010
\bibitem{di} Di Y, Schafer DW, Cumbie JS, and Chang JH. The NBP negative binomial model for assessing differential gene expression from RNA-Seq. Stat Appl Genet Mol Biol. 10: art24, 2011
...
\bibitem{glaus} Glaus P, Honkela A, and Rattray M. Identifying differentially expressed transcripts from RNA-seq data with biological variation. Bioinformatics 28(13): 1721-1728, 2012
\bibitem{hardcastle} Hardcastle TJ and Kelly KA. \Rpackage{baySeq}: empirical Bayesian methods for identifying differential expression in sequence count data. BMC Bioinformatics 11: 422, 2010
\bibitem{kadota} Kadota K, Nishiyama T, and Shimizu K. A normalization strategy for comparing tag count data. Algorithms Mol Biol. 7:5, 2012
\bibitem{robinson} Robinson MD, McCarthy DJ, and Smyth GK. \Rpackage{edgeR}: a Bioconductor package for differential expression analysis of digital gene expression data. Bioinformatics 26(1): 139-140, 2010
\bibitem{robinson2} Robinson MD and Oshlack A. A scaling normalization method for differential expression analysis of RNA-seq data. Genome Biol., 11: R25, 2010
\bibitem{robinson3} Robinson MD and Smyth GK. Small-sample estimation of negative binomial dispersion, with applications to SAGE data. Biostatistics 9: 321-332, 2008
\bibitem{sun} Sun J, Nishiyama T, Shimizu K, and Kadota K. \Rpackage{TCC}: a Bioconductor package for comparing tag count data with robust normalization strategies. submitted
\bibitem{mccarthy} McCarthy DJ, Chen Y, Smyth GK. Differential expression analysis of multifactor RNA-Seq experiments with respect to biological variation. Nucleic Acids Res. 40(10): 4288-4297, 2012
\end{thebibliography}
\end{document}
