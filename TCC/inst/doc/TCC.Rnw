...
\sectionfont{\bf\sectionrule{0pt}{0pt}{-1ex}{1pt}}
\subsectionfont{\bf}
...
(TbT; Kadota et al., 2012) for two-group RNA-seq data. The strategy 
...
Sun et al., submitted) provides users with a robust and accurate framework 
...
Kadota et al. (2012) recently reported that the normalization methods 
implemented in R packages (such as \Rpackage{edgeR} (Robinson et al., 2010), 
\Rpackage{DESeq} (Anders and Huber, 2010), and \Rpackage{baySeq} (Hardcastle 
and Kelly, 2010)) for differential expression (DE) analysis between samples 
...
et al., 2012), in which the TMM normalization method (Robinson and Oshlack, 
2010) is used in steps 1 and 3 and an empirical Bayesian method implemented 
in the \Rpackage{baySeq} package (Hardcastle and Kelly, 2010) is used in 
...
For example, the TbT normalization method (Kadota et al., 2012), which is 
a particular functionality of the \Rpackage{TCC} package (Sun et al., submitted), 
consists of the TMM normalization method (Robinson and Oshlack, 2010) implemented 
in the \Rpackage{edgeR} package (Robinson et al., 2010) and the empirical Bayesian 
method implemented in the \Rpackage{baySeq} package (Hardcastle and Kelly, 2010). 
...
(Robinson and Smyth, 2008) in \Rpackage{edgeR} coupled with TbT normalization. 
The \Rpackage{TCC} package was originally designed with the TbT normalization 
method, and the original study (Kadota et al., 2012) recommended this analysis 
...
and Kelly, 2010). Suggested citations are as follows: \Rpackage{TCC} (Sun et al., 
submitted), TbT (Kadota et al., 2012), TMM (Robinson and Oshlack, 2010), 
\Rpackage{baySeq} (Hardcastle and Kelly, 2010), and \Rpackage{edgeR} 
(Robinson et al., 2010). For details, see section \ref{section-3-1-1}.
...
follows: \Rpackage{TCC} (Sun et al., submitted), TMM (Robinson and Oshlack, 
2010), the exact test (Robinson and Smyth, 2008), and \Rpackage{edgeR} 
(Robinson et al., 2010). For details, see section \ref{section-3-1-3}.
...
citations are as follows: \Rpackage{TCC} (Sun et al., submitted) and 
\Rpackage{DESeq} (Anders and Huber, 2010). For details, 
...
to be accurately estimated to avoid spurious DE calls (Glaus et al., 2012). 
...
\Robject{samplesize = 10000}) is recommended (Hardcastle and Kelly, 2010).
...
and Kelly, 2010). Note also that \Rpackage{baySeq} outputs posterior 
...
real data (Dillies et al., 2012). This means we have to evaluate the 
potential performance of our DEGES-based methods using mainly simulation 
data. The \Rfunction{generateSimulationData} function generates simulation data under 
various conditions. This function can generate simulation data analyzed 
in the TbT paper (Kadota et al., 2012), and that means it enables other 
...
function. The output of the \Rfunction{generateSimulationData} function is stored 
...
(i.e., the \Robject{tcc\$count}) with the \Rfunction{generateSimulationData} function. 
...
DE method such as the exact test (Robinson and Smyth, 2008) and the 
...
The \Rfunction{generateSimulationData} function can generate simulation data 
...
set.seed(1000)
library(TCC)
tcc <- generateSimulationData(Ngene = 10000, PDEG = 0.34,
...
\item Anders S and Huber W. Differential expression analysis for sequence count data. Genome Biol. 11(10): R106, 2010
\item Di Y, Schafer DW, Cumbie JS, and Chang JH. The NBP negative binomial model for assessing differential gene expression from RNA-Seq. Stat Appl Genet Mol Biol. 10: art24, 2011
\item Dillies MA, Rau A, Aubert J, Hennequet-Antier C, Jeanmougin M, Servant N, Keime C, Marot G, Castel D, Estelle J, Guernec G, Jagla B, Jouneau L, Lalo\"e D, Le Gall C, Scha\"effer B, Le Crom S, Guedj M, Jaffr\'ezic F; on behalf of The French StatOmique Consortium. A comprehensive evaluation of normalization methods for Illumina high-throughput RNA sequencing data analysis. Brief Bioinform, in press
\item Glaus P, Honkela A, and Rattray M. Identifying differentially expressed transcripts from RNA-seq data with biological variation. Bioinformatics 28(13): 1721-1728, 2012
\item Hardcastle TJ and Kelly KA. \Rpackage{baySeq}: empirical Bayesian methods for identifying differential expression in sequence count data. BMC Bioinformatics 11: 422, 2010
\item Kadota K, Nishiyama T, and Shimizu K. A normalization strategy for comparing tag count data. Algorithms Mol Biol. 7:5, 2012
\item Robinson MD, McCarthy DJ, and Smyth GK. \Rpackage{edgeR}: a Bioconductor package for differential expression analysis of digital gene expression data. Bioinformatics 26(1): 139-140, 2010
\item Robinson MD and Oshlack A. A scaling normalization method for differential expression analysis of RNA-seq data. Genome Biol., 11: R25, 2010
\item Robinson MD and Smyth GK. Small-sample estimation of negative binomial dispersion, with applications to SAGE data. Biostatistics 9: 321-332, 2008
\item Sun J, Nishiyama T, Shimizu K, and Kadota K. \Rpackage{TCC}: a Bioconductor package for comparing tag count data with robust normalization strategies. submitted
\item McCarthy DJ, Chen Y, Smyth GK. Differential expression analysis of multifactor RNA-Seq experiments with respect to biological variation. Nucleic Acids Res. 40(10): 4288-4297, 2012
\end{thebibliography}
\end{document}
